...
def get_sem_sequence(fromfile, tofile):
    f1 = open(fromfile, 'r')
    lines = f1.readlines()
    f1.close()
    f = open(tofile, 'w')
    for text in lines:
        ...
        if 'sentences' in output:
            for i in range(len(output['sentences'])):
                ...
                for i in tag:
                    if i in a:
                        lst.append((i, a.find(i)))
                l = sorted(lst, key=lambda x:x[1])
                print l
                tmp = []
                for i in l:
                    tmp.append(tag.index(i[0]))
                rt.extend(tmp)
...
#by charactor a-z:
def get_sem_sequence2(fromfile, tofile):
    f1 = open(fromfile, 'r')
    lines = f1.readlines()
    f1.close()
    f = open(tofile, 'w')
...
if __name__ == '__main__':
    ...
    print abb
    get_sem_sequence2('sens_final.txt', 'seq_char.txt')
