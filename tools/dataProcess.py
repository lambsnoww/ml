...
''' 
    with nested(open("/Users/linxue/PycharmProjects/ml/resources/attr.txt", 'r'),
        open("/Users/linxue/PycharmProjects/ml/resources/dataout.txt", 'r')
        ) as (fp1, fp2):
        for l1 in fp1:
            l2 = fp2
            # get the Frame-related attributes
            # print type(l2)
            s1, s2 = fa.extractFrameAttribute(l2)
            ss = l1 + ',' + s1 + ',' + s2 + '\n'
            ff.write(ss)
    ff.close()
    fp1.close()
